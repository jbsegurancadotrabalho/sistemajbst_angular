...
import { PostNivelDeSeveridadeRiscosModel } from 'src/app/models/sms/PostNivelDeSeveridadeRiscosModel'; 
...
@Component({
  selector: 'app-criar-nivel-de-severidade-riscos',
  templateUrl: './criar-nivel-de-severidade-riscos.component.html',
  styleUrls: ['./criar-nivel-de-severidade-riscos.component.css']
})
export class CriarNivelDeSeveridadeRiscosComponent  implements OnInit {
  formNivel!: FormGroup;
  riscosForm!: FormGroup;
  id: string | null = null;
  ...
  criarNivelDeSeveridade() {
    ...
    if (this.formNivel.valid) {
      const novoRisco:  PostNivelDeSeveridadeRiscosModel = this.formNivel.value as PostNivelDeSeveridadeRiscosModel;
      console.log('Dados do formulário:', this.formNivel.value); // Verifique os valores do formulário
      this.NivelSeveridadeService.criarNivelDeSeveridadeRiscos(novoRisco).subscribe(
        response => {
