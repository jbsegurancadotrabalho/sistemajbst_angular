...
@Component({
  selector: 'app-consultar-profissional-saude',
  templateUrl: './consultar-profissional-saude.component.html',
  styleUrls: ['./consultar-profissional-saude.component.css']
})
export class ConsultarProfissionalSaudeComponent implements OnInit {
    ...
    if (this.id) {
      this.CredenciadoService.consultarCredenciadoPorId(this.id).subscribe(
        (data: GetCredenciadosModel) => {
          ...
          this.credenciados = data.profissionalsaude; // Atualizar a lista de exames
